...
class FlipFlopGenerator:
    @staticmethod
    def generate(seed, size=20, crossover=None, mutator=None):
        np.random.seed(seed)
        problem = FlipFlopOpt(length=size, crossover=crossover, mutator=mutator)
        return problem
...
class FlipFlopOpt(CustomDiscreteOpt):
